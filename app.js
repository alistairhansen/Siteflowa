...
// ── TERMS OF SERVICE ──────────────────────────────────
const TOS_TEXT = `SITEFLOWA INDEPENDENT CONTRACTOR TERMS

By accessing, using, or being granted any level of access to Siteflowa systems, dashboards, tools, client interfaces, or business resources, you expressly agree that you are acting as an independent contractor and not as an employee, agent, or partner of Siteflowa.
...
You further agree that any sales, leads, or client acquisitions generated by you on behalf of Siteflowa may qualify you for commission-based compensation as determined solely by Siteflowa, and that such commission is only payable after successful payment confirmation from the client and any applicable refund or dispute periods have passed.

Siteflowa reserves the right to modify, adjust, or revoke commission structures, payment terms, and eligibility criteria at any time at its sole discretion; however, Siteflowa agrees to provide no less than fourteen (14) days prior notice to all active contractors before any changes to commission rates or structures take effect.

You agree that Siteflowa may, at any time and for any reason, restrict, suspend, or permanently revoke your access to dashboards, internal systems, client tools, or any related infrastructure, with or without notice.
...
You must not copy, reverse engineer, distribute, or misuse any part of Siteflowa systems, code, data, or client information, and you agree to maintain strict confidentiality regarding all non-public business information, including but not limited to pricing, systems, clients, processes, and software.
...
By continuing to access or use Siteflowa systems or by performing any sales or business activities on behalf of Siteflowa, you confirm that you have read, understood, and agreed to be bound by these terms in full.`

function checkTOS(email, role) {
  if (role !== 'admin' && role !== 'manager') return
  const tosKey = 'siteflowa_tos_' + email
  ...
  modal.innerHTML = `
    <div style="background:white;border-radius:18px;max-width:620px;width:100%;max-height:90vh;display:flex;flex-direction:column;">
      ...
      <div style="padding:16px 32px;border-top:1px solid var(--border);font-size:12px;color:var(--ink-muted);">
        Accepted on: ${localStorage.getItem('siteflowa_tos_'+email) ? new Date(localStorage.getItem('siteflowa_tos_'+email)).toLocaleDateString('en-CA',{year:'numeric',month:'long',day:'numeric'}) : 'Not yet accepted'}
      </div>
    </div>
  `
  document.body.appendChild(modal)
  ...
  tbody.innerHTML=clients.map(c=>`
    ...
    <tr class="client-detail-row" id="mgr-detail-${c.id}"><td colspan="7"><div class="client-detail-inner">
      <div class="detail-grid">
        ...
        <div class="detail-item"><div class="dl">Subdomain</div><div class="dv">${c.subdomain?'<a href="/client/'+c.subdomain+'" target="_blank" style="color:var(--accent);">'+c.subdomain+'.siteflowa.com</a>':'--'}</div></div>
