...
const _lm = document.getElementById('login-modal'); if(_lm) _lm.addEventListener('click',function(e){if(e.target===this)closeLogin()})
...
// ══════════════════════════════════════════════════════
// CLIENT INQUIRY MANAGEMENT & FORM SYSTEM
// ══════════════════════════════════════════════════════
async function submitInquiryForm() {
  const businessName = document.getElementById('inquiry-form-business')?.value?.trim()
  const email = document.getElementById('inquiry-form-email')?.value?.trim()
  const phone = document.getElementById('inquiry-form-phone')?.value?.trim()
  const plan = document.getElementById('inquiry-form-plan')?.value || 'standard'
  ...
  const formData = {
    business_type: document.getElementById('inquiry-form-type')?.value,
    main_goal: document.getElementById('inquiry-form-goal')?.value,
    sections: Array.from(document.querySelectorAll('input[name="inquiry-sections"]:checked')).map(i => i.value),
    style: document.getElementById('inquiry-form-style')?.value
  }
  ...
  try {
    const res = await fetch(API + '/inquiry-form', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ business_name: businessName, email, phone, plan, questions_data: formData })
    })
    const d = await res.json()
    if (d.inquiry) {
      alert('Thank you! We\'ll review your inquiry and get back to you within 24 hours.')
      showPage('home')
    } else {
...
// Load inquiries for staff
async function loadInquiries() {
  const wrap = document.getElementById('inquiries-list-wrap')
  if (!wrap) return
  try {
    ...
    if (!inquiries.length) {
      wrap.innerHTML = '<p style="color:var(--ink-muted);font-size:14px;">No inquiries available.</p>'
      return
    }
    
    wrap.innerHTML = inquiries.map(inq => `
      <div style="border:1px solid var(--border);border-radius:var(--radius-lg);padding:16px 20px;margin-bottom:12px;">
        <div style="display:flex;justify-content:space-between;align-items:start;gap:16px;margin-bottom:12px;">
          <div>
            <div style="font-weight:600;font-size:15px;">${inq.business_name}</div>
            <div style="font-size:13px;color:var(--ink-muted);">${inq.email}</div>
            <div style="font-size:13px;color:var(--ink-muted);">Plan: <strong>${inq.plan}</strong></div>
            <div style="font-size:12px;color:var(--ink-muted);margin-top:6px;">Submitted: ${new Date(inq.created_at).toLocaleDateString()}</div>
          </div>
          <div style="text-align:right;">
            ${inq.claimed_by ? `<div style="background:var(--accent-light);color:var(--accent);padding:6px 12px;border-radius:20px;font-size:11px;font-weight:700;margin-bottom:8px;">CLAIMED</div>` : ''}
            <button class="dash-save" onclick="viewAndClaimInquiry('${inq.id}')" style="font-size:13px;padding:10px 16px;">${inq.claimed_by ? 'View' : 'Claim Lead'}</button>
          </div>
        </div>
      </div>
    `).join('')
...
async function viewAndClaimInquiry(id) {
  try {
    ...
    modal.style.cssText = 'position:fixed;inset:0;z-index:500;background:rgba(15,17,23,0.85);display:flex;align-items:center;justify-content:center;padding:20px;'
    modal.innerHTML = `
      <div style="background:white;border-radius:18px;max-width:600px;width:100%;max-height:90vh;display:flex;flex-direction:column;">
        <div style="padding:28px 32px 20px;border-bottom:1px solid var(--border);">
          <h2 style="font-family:var(--serif);font-size:24px;margin-bottom:8px;">${inq.business_name}</h2>
          <div style="display:flex;gap:16px;flex-wrap:wrap;">
            <div><div style="font-size:11px;color:var(--ink-muted);margin-bottom:4px;">EMAIL</div><div style="font-weight:600;">${inq.email}</div></div>
            <div><div style="font-size:11px;color:var(--ink-muted);margin-bottom:4px;">PHONE</div><div style="font-weight:600;">${inq.phone || 'Not provided'}</div></div>
            <div><div style="font-size:11px;color:var(--ink-muted);margin-bottom:4px;">PLAN</div><div style="font-weight:600;">${inq.plan}</div></div>
          </div>
        </div>
        <div style="padding:24px 32px;overflow-y:auto;flex:1;">
          <div style="background:var(--cream);border:1px solid var(--border);border-radius:var(--radius);padding:16px;margin-bottom:16px;">
            <h3 style="font-size:14px;font-weight:600;margin-bottom:12px;">Inquiry Details</h3>
            <pre style="white-space:pre-wrap;word-break:break-word;font-family:var(--sans);font-size:13px;color:var(--ink-light);">${JSON.stringify(inq.form_data, null, 2)}</pre>
          </div>
        </div>
        <div style="padding:20px 32px;border-top:1px solid var(--border);display:flex;gap:12px;">
          ${!inq.claimed_by ? `<button class="dash-save" onclick="claimInquiry('${inq.id}')">Claim This Lead</button>` : '<span style="color:var(--accent);font-weight:600;">✓ Already claimed by you</span>'}
          <button onclick="this.closest('div').parentElement.remove()" class="btn-secondary" style="margin-left:auto;">Close</button>
        </div>
      </div>
    `
    document.body.appendChild(modal)
...
async function claimInquiry(id) {
  try {
    const res = await fetch(API + '/inquiry/' + id + '/claim', {
      method: 'POST',
      headers: { 'Authorization': 'Bearer ' + getToken() }
    })
    const d = await res.json()
    if (d.inquiry) {
      alert('Inquiry claimed! The client\'s email is now only visible to you.')
...
// ══════════════════════════════════════════════════════
// CONTRACTOR & MANAGER PAY PERIODS
// ══════════════════════════════════════════════════════
async function loadContractorEarnings() {
  const wrap = document.getElementById('contractor-earnings-wrap')
  if (!wrap) return
  try {
    const res = await fetch(API + '/admin/contractor-earnings', {
      headers: { 'Authorization': 'Bearer ' + getToken() }
    })
    const d = await res.json()
    ...
    if (!earnings.length) {
      wrap.innerHTML = '<p style="color:var(--ink-muted);font-size:14px;">No contractor earnings yet.</p>'
      return
    }
    
    const chartData = {
      labels: earnings.map(e => e.email.split('@')[0]),
      earnings: earnings.map(e => parseFloat(e.net_amount) || 0),
      revenue: earnings.map(e => parseFloat(e.total_earned) || 0)
    }
    
    wrap.innerHTML = `
      <canvas id="contractor-earnings-chart" style="max-height:300px;margin-bottom:24px;"></canvas>
      <div style="background:var(--cream);border:1px solid var(--border);border-radius:var(--radius-lg);overflow:hidden;">
        <table style="width:100%;border-collapse:collapse;">
          <thead>
            <tr style="border-bottom:1px solid var(--border);">
              <th style="padding:12px 16px;text-align:left;font-weight:600;font-size:13px;">Contractor</th>
              <th style="padding:12px 16px;text-align:center;font-weight:600;font-size:13px;">Websites</th>
              <th style="padding:12px 16px;text-align:right;font-weight:600;font-size:13px;">Total Earned</th>
              <th style="padding:12px 16px;text-align:right;font-weight:600;font-size:13px;">Net (After Commission)</th>
            </tr>
          </thead>
          <tbody>
            ${earnings.map(e => `
              <tr style="border-bottom:1px solid var(--border);">
                <td style="padding:12px 16px;font-size:13px;">${e.email}</td>
                <td style="padding:12px 16px;text-align:center;font-size:13px;">${e.websites_created}</td>
                <td style="padding:12px 16px;text-align:right;font-size:13px;font-weight:600;">$${parseFloat(e.total_earned).toFixed(2)}</td>
                <td style="padding:12px 16px;text-align:right;font-size:13px;font-weight:600;color:var(--accent);">$${parseFloat(e.net_amount).toFixed(2)}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
    ...
    setTimeout(() => {
      const ctx = document.getElementById('contractor-earnings-chart')
      if (ctx && typeof Chart !== 'undefined') {
        new Chart(ctx, {
          ...
          data: {
            ...
            datasets: [
              { label: 'Net Earnings', data: chartData.earnings, backgroundColor: '#9c4dcc' },
              { label: 'Total Revenue', data: chartData.revenue, backgroundColor: '#ce93d8' }
            ]
          },
...
async function loadManagerPerformance() {
  const wrap = document.getElementById('manager-performance-wrap')
  if (!wrap) return
  try {
    const res = await fetch(API + '/admin/manager-performance', {
      headers: { 'Authorization': 'Bearer ' + getToken() }
    })
    const d = await res.json()
    const stats = d.stats || []
    ...
    if (!stats.length) {
      wrap.innerHTML = '<p style="color:var(--ink-muted);font-size:14px;">No manager data yet.</p>'
      return
    }
    ...
    wrap.innerHTML = `
      <div style="background:var(--cream);border:1px solid var(--border);border-radius:var(--radius-lg);overflow:hidden;">
        <table style="width:100%;border-collapse:collapse;">
          <thead>
            <tr style="border-bottom:1px solid var(--border);">
              <th style="padding:12px 16px;text-align:left;font-weight:600;font-size:13px;">Manager</th>
              <th style="padding:12px 16px;text-align:center;font-weight:600;font-size:13px;">Websites</th>
              <th style="padding:12px 16px;text-align:center;font-weight:600;font-size:13px;">Revenue</th>
              <th style="padding:12px 16px;text-align:right;font-weight:600;font-size:13px;">Commission Paid</th>
              <th style="padding:12px 16px;text-align:center;font-weight:600;font-size:13px;">Action</th>
            </tr>
          </thead>
          <tbody>
            ${stats.map(s => `
              <tr style="border-bottom:1px solid var(--border);">
                <td style="padding:12px 16px;font-size:13px;">${s.email}</td>
                <td style="padding:12px 16px;text-align:center;font-size:13px;">${s.websites_created}</td>
                <td style="padding:12px 16px;text-align:center;font-size:13px;">$${parseFloat(s.revenue_brought_in).toFixed(2)}</td>
                <td style="padding:12px 16px;text-align:right;font-size:13px;font-weight:600;">$${parseFloat(s.commission_paid).toFixed(2)}</td>
                <td style="padding:12px 16px;text-align:center;"><button class="action-btn" onclick="showPayManagerModal('${s.id}','${s.email}')" style="font-size:12px;padding:6px 12px;">Close Period</button></td>
              </tr>
            `).join('')}
          </tbody>
        </table>
...
function showPayManagerModal(managerId, email) {
  const modal = document.createElement('div')
  modal.style.cssText = 'position:fixed;inset:0;z-index:500;background:rgba(15,17,23,0.85);display:flex;align-items:center;justify-content:center;padding:20px;'
  modal.innerHTML = `
    <div style="background:white;border-radius:18px;max-width:480px;width:100%;">
      <div style="padding:28px 32px 20px;border-bottom:1px solid var(--border);">
        <h2 style="font-family:var(--serif);font-size:24px;margin-bottom:8px;">Close Pay Period</h2>
        <p style="font-size:13px;color:var(--ink-muted);">Pay ${email} and send receipt.</p>
      </div>
      <div style="padding:24px 32px;">
        <div class="dash-field"><label>Amount to Pay</label><input type="number" id="pay-amount" placeholder="0.00" step="0.01"></div>
        <div class="dash-field"><label>Period Start Date</label><input type="date" id="pay-start"></div>
        <div class="dash-field"><label>Period End Date</label><input type="date" id="pay-end"></div>
      </div>
      <div style="padding:20px 32px;border-top:1px solid var(--border);display:flex;gap:12px;">
        <button class="dash-save" onclick="processManagerPayment('${managerId}','${email}')" style="flex:1;">Process Payment</button>
        <button onclick="this.closest('div').parentElement.remove()" class="btn-secondary">Cancel</button>
      </div>
    </div>
  `
  document.body.appendChild(modal)
...
async function processManagerPayment(managerId, email) {
  const amount = parseFloat(document.getElementById('pay-amount')?.value)
  const periodStart = document.getElementById('pay-start')?.value
  const periodEnd = document.getElementById('pay-end')?.value
  ...
  try {
    const res = await fetch(API + '/admin/pay-period-complete', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + getToken() },
      body: JSON.stringify({ staff_id: managerId, staff_role: 'manager', amount, period_start: periodStart, period_end: periodEnd })
    })
    const d = await res.json()
    if (d.message) {
      alert('Payment processed and receipt sent to ' + email)
      document.querySelectorAll('div[style*="position:fixed"]').forEach(m => m.remove())
...
async function loadClientsList() {
  ...
  try {
    ...
    if (!clients.length) {
      wrap.innerHTML = '<p style="color:var(--ink-muted);font-size:14px;">No clients yet.</p>'
      return
    }
    ...
    wrap.innerHTML = `
      <div style="background:var(--cream);border:1px solid var(--border);border-radius:var(--radius-lg);overflow:hidden;">
        <table style="width:100%;border-collapse:collapse;">
          <thead>
            <tr style="border-bottom:1px solid var(--border);">
              <th style="padding:12px 16px;text-align:left;font-weight:600;font-size:13px;">Business Name</th>
              <th style="padding:12px 16px;text-align:left;font-weight:600;font-size:13px;">Client Email</th>
              <th style="padding:12px 16px;text-align:left;font-weight:600;font-size:13px;">Creator</th>
              <th style="padding:12px 16px;text-align:center;font-weight:600;font-size:13px;">Created Date</th>
              <th style="padding:12px 16px;text-align:center;font-weight:600;font-size:13px;">Action</th>
            </tr>
          </thead>
          <tbody>
            ${clients.map(c => `
              <tr style="border-bottom:1px solid var(--border);">
                <td style="padding:12px 16px;font-size:13px;">${c.business_name || 'N/A'}</td>
                <td style="padding:12px 16px;font-size:13px;">${c.client_email || 'N/A'}</td>
                <td style="padding:12px 16px;font-size:13px;">${c.creator_email || 'Unassigned'}</td>
                <td style="padding:12px 16px;text-align:center;font-size:13px;">${new Date(c.created_at).toLocaleDateString()}</td>
                <td style="padding:12px 16px;text-align:center;"><button class="action-btn" onclick="showTransferClientModal('${c.id}')" style="font-size:12px;padding:6px 12px;">Transfer</button></td>
              </tr>
            `).join('')}
          </tbody>
        </table>
...
async function showTransferClientModal(websiteId) {
  try {
    ...
    modal.innerHTML = `
      <div style="background:white;border-radius:18px;max-width:480px;width:100%;">
        <div style="padding:28px 32px 20px;border-bottom:1px solid var(--border);">
          <h2 style="font-family:var(--serif);font-size:24px;margin-bottom:8px;">Transfer Client</h2>
          <p style="font-size:13px;color:var(--ink-muted);">Assign this client to a different contractor or manager.</p>
        </div>
        <div style="padding:24px 32px;">
          <div class="dash-field">
            <label>New Creator</label>
            <select id="transfer-staff-select">
              <option value="">Select a contractor or manager...</option>
              ${staff.map(s => `<option value="${s.id}">${s.email} (${s.role})</option>`).join('')}
            </select>
          </div>
        </div>
        <div style="padding:20px 32px;border-top:1px solid var(--border);display:flex;gap:12px;">
          <button class="dash-save" onclick="transferClient('${websiteId}')" style="flex:1;">Transfer</button>
          <button onclick="this.closest('div').parentElement.remove()" class="btn-secondary">Cancel</button>
        </div>
      </div>
    `
...
async function transferClient(websiteId) {
  const newCreatorId = document.getElementById('transfer-staff-select')?.value
  if (!newCreatorId) {
  ...
  try {
    const res = await fetch(API + '/admin/transfer-client', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + getToken() },
      body: JSON.stringify({ website_id: websiteId, new_creator_id: newCreatorId })
    })
    const d = await res.json()
    if (d.website) {
  ...
  modal.innerHTML = `
    <div style="background:white;border-radius:18px;max-width:800px;width:100%;max-height:90vh;display:flex;flex-direction:column;">
      ...
      <div style="padding:20px 32px;border-top:1px solid var(--border);display:flex;gap:12px;justify-content:flex-end;">
        <button onclick="this.closest('div').parentElement.remove()" class="btn-secondary">Close</button>
      </div>
    </div>
  `
  document.body.appendChild(modal)
