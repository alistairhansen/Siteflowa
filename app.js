...
window.addEventListener('load',()=>{
  ...
  if(params.get('payment')==='success'){
    // Final payment confirmed - activate account and load full dashboard
    const token=getToken()
    if(token){
      // Belt-and-suspenders activate call (webhook may have already done this)
      fetch(API+'/activate-account',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token}})
        .catch(()=>{})
        .finally(()=>{
          // Always re-fetch fresh dashboard data from server
          fetch(API+'/my-dashboard',{headers:{'Authorization':'Bearer '+token}})
            .then(r=>r.json())
            .then(data=>{
              const email=localStorage.getItem('wc_email')
              alert('Payment successful! Your website is now live.')
...
async function loadHoldingPage(clientData, websiteData) {
  ...
  if (stage === 'account_created' || !clientData.deposit_paid) {
    ...
    wrap.innerHTML = '<div style="font-size:52px;margin-bottom:20px;">🔨</div>'
      ...
      + '<p style="font-size:16px;color:var(--ink-light);line-height:1.7;margin-bottom:20px;">Your deposit has been received. If you haven\'t already, fill in your website brief so we know exactly what to build.</p>'
      + '<button onclick="showPage(\'assetform\');initBriefForm(\'' + plan + '\')" style="background:var(--accent);color:white;border:none;padding:12px 28px;border-radius:10px;font-family:var(--sans);font-size:15px;font-weight:500;cursor:pointer;margin-bottom:24px;">Fill in your website brief \u2192</button>'
      ...
      + '<div style="background:var(--cream);padding:12px 16px;border-bottom:1px solid var(--border);font-size:13px;font-weight:600;">\u{1F4AC} Chat with us</div>'
  ...
  } else if (stage === 'brief_submitted') {
    var chatClientId2 = JSON.parse(atob(getToken().split('.')[1])).id
    wrap.innerHTML = '<div style="font-size:52px;margin-bottom:20px;">\u2705</div>'
      + '<h2 style="font-family:var(--serif);font-size:clamp(26px,4vw,38px);margin-bottom:14px;">Brief received!</h2>'
      + '<p style="font-size:16px;color:var(--ink-light);line-height:1.7;margin-bottom:20px;">We\'ve got your website brief and our team is now working on it. We\'ll send you an email as soon as it\'s ready to preview.</p>'
      + '<div style="background:white;border:1px solid var(--border);border-radius:var(--radius-lg);overflow:hidden;margin-bottom:24px;max-width:500px;margin-left:auto;margin-right:auto;">'
      + '<div style="background:var(--cream);padding:12px 16px;border-bottom:1px solid var(--border);font-size:13px;font-weight:600;">\u{1F4AC} Chat with us</div>'
      + '<div id="client-chat-messages" style="height:220px;overflow-y:auto;padding:14px;display:flex;flex-direction:column;gap:8px;"></div>'
      + '<div style="padding:10px 12px;border-top:1px solid var(--border);display:flex;gap:8px;">'
      + '<input type="text" id="client-chat-input" placeholder="Ask us anything about your website..." style="flex:1;padding:8px 12px;border:1px solid var(--border);border-radius:var(--radius);font-family:var(--sans);font-size:13px;">'
      + '<button onclick="sendClientMessage(\'' + chatClientId2 + '\')" style="background:var(--accent);color:white;border:none;padding:8px 16px;border-radius:var(--radius);font-family:var(--sans);font-size:13px;cursor:pointer;">Send</button>'
      + '</div></div>'
      + '<button onclick="showPage(\'assetform\');initBriefForm(\'' + plan + '\')" style="background:white;border:1px solid var(--border);color:var(--ink);padding:10px 22px;border-radius:8px;font-family:var(--sans);font-size:13px;cursor:pointer;margin-top:8px;">Edit / resubmit brief</button>'
    document.getElementById('client-chat-input').addEventListener('keydown', function(e){ if(e.key==='Enter'){ sendClientMessage(chatClientId2) } })
    loadClientMessages()
    setInterval(loadClientMessages, 8000)
    ...
    wrap.innerHTML = chats.map(c => `
      <div style="border:1px solid var(--border);border-radius:var(--radius-lg);padding:16px 20px;margin-bottom:10px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;${parseInt(c.unread)>0?'background:var(--accent-light);border-color:var(--accent);':''}">
        <div>
          ...
          <div style="font-size:11px;color:var(--ink-muted);">Stage: ${({account_created:'Account created',deposit_paid:'Deposit paid',building:'Building',brief_pending:'Brief pending',brief_submitted:'Brief submitted ✅',preview_ready:'Preview ready',launched:'Launched 🚀'}[c.onboarding_stage]||c.onboarding_stage)} &middot; Last message: ${c.last_message?new Date(c.last_message).toLocaleDateString('en-CA',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'}):'None'}</div>
        </div>
